...
namespace ImageResizer
{
    class Program
    {
        static async Task Main(string[] args)
        {
            ...
            long totalms=0;
            var runtimes = int.Parse(ConfigurationManager.AppSettings["runtimes"]) ;
            ...
            specificName = "plinqasync";
            var plinqdestinationPath = $"{destinationPath}{specificName}";
            totalms = 0;

            for (int i = 0; i < runtimes; i++)
            {
                imageProcess.Clean(plinqdestinationPath);
            ...
            var plinqAsyncAvg = totalms / runtimes;
